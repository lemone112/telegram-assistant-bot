...
async function applyDealStage(env: Env, actorTelegramId: number, draftDbId: string, dealId: string, stageInput: string) {
  ...
  if (!attioConn) {
    throw new Error(
      "Composio Attio connection id is not configured. Set it via /admin composio attio <connected_account_id>"
    );
  }
...
async function ensureLinearKickoff(
  ...
  // 0) Read deal snapshot from Attio (for project naming + stage verification)
  const dealRecord = await composioExecute(env, {
    tool_slug: "ATTIO_GET_RECORD",
    connected_account_id: attioConn,
    arguments: {
      object_type: "deals",
      record_id: attioDealId,
    },
  });

  // Best-effort extraction (schema may evolve); fallback to null.
  const dealName =
    (dealRecord as any)?.data?.values?.name?.[0] ??
    (dealRecord as any)?.values?.name?.[0] ??
    (dealRecord as any)?.data?.name ??
    (dealRecord as any)?.name ??
    null;

  const stageName =
    (dealRecord as any)?.data?.values?.stage?.[0] ??
    (dealRecord as any)?.values?.stage?.[0] ??
    (dealRecord as any)?.data?.stage?.[0] ??
    (dealRecord as any)?.stage?.[0] ??
    null;

  // Company name is not guaranteed to be present without extra deref; keep best-effort.
  const companyName =
    (dealRecord as any)?.data?.values?.associated_company?.[0]?.name ??
    (dealRecord as any)?.values?.associated_company?.[0]?.name ??
    null;

  // 1) Create/reuse Linear Project (idempotent via bot.deal_linear_links)
  ...
  if (linearProjectId) {
    projectReused = true;
    await audit(env, draftDbId, "linear.project.reused", "info", {
      actor_telegram_user_id: actorTelegramId,
      attio_deal_id: attioDealId,
      linear_project_id: linearProjectId,
    });
  } else {
    const nameParts = [companyName, dealName].filter((x) => typeof x === "string" && x.trim().length);
    projectName = nameParts.length ? nameParts.join(" — ") : `Deal — ${attioDealId}`;

    if (!nameParts.length) {
      await audit(env, draftDbId, "linear.project.name.fallback", "info", {
        actor_telegram_user_id: actorTelegramId,
        attio_deal_id: attioDealId,
        deal_name: dealName,
    ...
    const createdProject = await composioExecute(env, {
      tool_slug: "LINEAR_CREATE_LINEAR_PROJECT",
      connected_account_id: linearConn,
      arguments: {
        name: projectName,
        team_ids: [linearTeamId],
        description: `Attio record: ${attioDealId}`.slice(0, 255),
      },
    });

    linearProjectId =
      (createdProject as any)?.id ??
      (createdProject as any)?.data?.id ??
      (createdProject as any)?.project?.id ??
      (createdProject as any)?.data?.project?.id ??
      null;

    if (!linearProjectId) {
      throw new Error(`Linear project create returned no id: ${JSON.stringify(createdProject)}`);
    }
    ...
    await audit(env, draftDbId, "linear.project.created", "info", {
      actor_telegram_user_id: actorTelegramId,
      attio_deal_id: attioDealId,
      linear_project_id: linearProjectId,
      project_name: projectName,
  ...
  // Fetch project URL (best-effort) for backlink + result
  let linearProjectUrl: string | null = null;
  try {
    const proj = await composioExecute(env, {
      tool_slug: "LINEAR_GET_LINEAR_PROJECT",
      connected_account_id: linearConn,
      arguments: { project_id: linearProjectId },
    });
    linearProjectUrl =
      (proj as any)?.url ??
      (proj as any)?.data?.url ??
      (proj as any)?.project?.url ??
      (proj as any)?.data?.project?.url ??
      null;
  } catch (e) {
    await audit(
      env,
      draftDbId,
      "linear.project.get.failed",
      "error",
      {
        actor_telegram_user_id: actorTelegramId,
        attio_deal_id: attioDealId,
  ...
  // 2) Create kickoff issues inside project (idempotent via bot.project_template_tasks)
  const created: any[] = [];
  const skipped: any[] = [];
  ...
  for (const t of KICKOFF_TEMPLATE_TASKS) {
    ...
    if ((existingTask as any)?.linear_issue_id) {
      skipped.push({
        ...
        identifier: (existingTask as any)?.linear_issue_identifier ?? null,
      });
      continue;
    }
  ...
  await audit(env, draftDbId, "linear.kickoff.issues.summary", "info", {
    actor_telegram_user_id: actorTelegramId,
    attio_deal_id: attioDealId,
    linear_project_id: linearProjectId,
    created_count: created.length,
    skipped_count: skipped.length,
    created_identifiers: created.map((x) => x.identifier).filter(Boolean),
  });

  // 3) Backlink note in Attio (best-effort idempotency via marker scan)
  const marker = `linear_project_id=${linearProjectId}`;
  const noteTitle = "Linear kickoff";

  let noteCreated = false;
  let noteId: string | null = null;
  try {
    const notes = await composioExecute(env, {
      tool_slug: "ATTIO_LIST_NOTES",
      connected_account_id: attioConn,
      arguments: {
        parent_object: "deals",
        parent_record_id: attioDealId,
        limit: 50,
        offset: 0,
      },
    });

    const existingNotes: any[] = (notes as any)?.data ?? (notes as any)?.notes ?? (notes as any)?.data?.data ?? [];
    const found = (existingNotes || []).find((n) => {
      const content = n?.content ?? n?.body ?? n?.markdown ?? "";
      return typeof content === "string" && content.includes(marker);
    });

    if (found?.id) {
      noteId = found.id;
      await audit(env, draftDbId, "attio.note.skipped", "info", {
        actor_telegram_user_id: actorTelegramId,
        attio_deal_id: attioDealId,
        reason: "existing marker",
        note_id: noteId,
        marker,
      });
    } else {
      const issueList = created.map((x) => x.identifier).filter(Boolean);
      const lines = [
        `Linear project: ${linearProjectUrl ?? linearProjectId}`,
        `Created kickoff issues: ${created.length}`,
        issueList.length ? `Issues: ${issueList.join(", ")}` : null,
        "",
        `Marker: ${marker}`,
      ].filter(Boolean);

      const note = await composioExecute(env, {
        tool_slug: "ATTIO_CREATE_NOTE",
        connected_account_id: attioConn,
        arguments: {
          parent_object: "deals",
          parent_record_id: attioDealId,
      ...
      noteId = (note as any)?.id ?? (note as any)?.data?.id ?? null;
      noteCreated = true;
      await audit(env, draftDbId, "attio.note.created", "info", {
        actor_telegram_user_id: actorTelegramId,
        attio_deal_id: attioDealId,
        note_id: noteId,
        marker,
  ...
  // Stage check observability (non-blocking) — keep snapshot in result
  const stageOk = stageName === "Выиграно" || stageName === "выиграно";
  ...
  return {
    linear_project: {
      ...
      name: projectName,
      reused: projectReused,
    },
    issues: {
      created_count: created.length,
      skipped_count: skipped.length,
      created_issue_identifiers: created.map((x) => x.identifier).filter(Boolean),
    },
    attio: {
      deal_id: attioDealId,
    ...
    case "deal": {
      ...
      if (sub === "won") {
        ...
        await tgCall(env, "sendMessage", {
          ...
          text:
            `Draft создан.\n\nСделка: ${dealId}\n\n` +
            "Apply переведёт стадию в «Выиграно», создаст Linear Project + 12 kickoff задач и оставит backlink Note в Attio.\n\n" +
            "Применить/Отмена?",
          reply_markup: JSON.stringify(buildInlineKeyboard(draftDbId)),
        });
        return;
...
async function handleCallback(env: Env, cb: NonNullable<TelegramUpdate["callback_query"]>) {
  ...
  try {
    ...
    if (first?.type === "deal.won") {
      ...
      const result = {
        ok: true,
        linear_project_id: (kickoff as any)?.linear_project?.id ?? null,
        linear_project_url: (kickoff as any)?.linear_project?.url ?? null,
        created_issues_count: (kickoff as any)?.issues?.created_count ?? 0,
        created_issue_identifiers: (kickoff as any)?.issues?.created_issue_identifiers ?? [],
        kickoff,
      };

      await client.schema(sch).from("drafts").update({ status: "APPLIED" } as any).eq("id", draftDbId);
      await finishApplyAttempt(env, attemptId, result, null);
      ...
      await tgCall(env, "sendMessage", {
        ...
        text:
          ...
          `Kickoff задач создано: ${(kickoff as any)?.issues?.created_count ?? 0}.\n` +
          ((kickoff as any)?.linear_project?.url ? `Linear: ${(kickoff as any).linear_project.url}\n` : ""),
      });
      return;
    }
